...
class DefaultMoveComparatorTest {
	private static final class ChessLibDefaultMoveComparator extends DefaultMoveComparator<Move, Board> implements ChessLibAdapter {
	...
	void test() {
		...
		final DefaultMoveComparator<Move, Board> cmp = new ChessLibDefaultMoveComparator(board);
...
//		Arrays.stream(moves).map(m -> m.toString()+":"+cmp.evaluate(m)).forEach(System.out::println);
		...
		board.loadFromFen("3n4/4P3/8/5P2/1p6/QK5k/1P6/8 b - - 0 1");
